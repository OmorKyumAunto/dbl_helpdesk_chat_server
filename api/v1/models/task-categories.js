...
let getList = async (user_id) => {
    return new Promise((resolve, reject) => {
        connectionDblystem.query(queries.getList(),[user_id], (error, result, fields) => {
...
let getByTitle = async (title = "",user_id = 0) => {
    return new Promise((resolve, reject) => {
        connectionDblystem.query(queries.getByTitle(), [title,user_id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getById = async (id = 0,user_id = 0) => {
    return new Promise((resolve, reject) => {
        connectionDblystem.query(queries.getById(), [id,user_id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
