...
let combineReportTicketTaskCount = (start_date, end_date, user_id) => {
  ...
  return `
    SELECT
      -- Total Counts
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' THEN 1 ELSE 0 END) AS total_ticket,
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' THEN 1 ELSE 0 END) AS total_task,
      COUNT(*) AS total_ticket_task,

      -- Overdue Counts
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' AND is_overdue = 1 THEN 1 ELSE 0 END) AS ticket_overdue_count,
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' AND is_overdue = 1 THEN 1 ELSE 0 END) AS task_overdue_count,
      SUM(CASE WHEN is_overdue = 1 THEN 1 ELSE 0 END) AS total_ticket_task_overdue_count,

      -- In-Time Counts
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' AND is_overdue = 0 THEN 1 ELSE 0 END) AS in_time_ticket,
      SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' AND is_overdue = 0 THEN 1 ELSE 0 END) AS in_time_task,

      -- Total Ticket Time Sum
      CONCAT(
        FLOOR(SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) / 3600), ':',
        LPAD(FLOOR((SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) % 3600) / 60), 2, '0'), ':',
        LPAD(SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) % 60, 2, '0')
      ) AS total_ticket_time_sum,

      -- Total Task Time Sum
      CONCAT(
        FLOOR(SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) / 3600), ':',
        LPAD(FLOOR((SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) % 3600) / 60), 2, '0'), ':',
        LPAD(SUM(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END) % 60, 2, '0')
      ) AS total_task_time_sum,
      ...
      -- Avg Ticket Time (HH:MM:SS)
      SEC_TO_TIME(AVG(CASE WHEN source COLLATE utf8mb4_general_ci = 'ticket' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END)) AS avg_ticket_time,

      -- Avg Task Time (HH:MM:SS)
      SEC_TO_TIME(AVG(CASE WHEN source COLLATE utf8mb4_general_ci = 'task' THEN TIMESTAMPDIFF(SECOND, created_at, updated_at) END)) AS avg_task_time,

      -- Combined Avg (HH:MM:SS)
      SEC_TO_TIME(AVG(TIMESTAMPDIFF(SECOND, created_at, updated_at))) AS avg_ticket_task_time

    FROM dbl_database.combine_report_view
    WHERE ${searchCondition} AND source COLLATE utf8mb4_general_ci IN ('ticket', 'task');
  `;
};
