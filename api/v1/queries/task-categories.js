...
let getList = () => {
    return `SELECT * FROM ${table_name} WHERE created_by = ? AND status = 1 ORDER BY id DESC`;
}
...
let getByTitle = () => {
    return `SELECT * FROM ${table_name} where  title = ? and created_by = ? and status = 1`;
}

let getById = () => {
    return `SELECT * FROM ${table_name} where  id = ? and created_by = ? and status = 1 `;
}
