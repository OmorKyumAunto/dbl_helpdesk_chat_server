...
const queries = require('../queries/group-location');
...
let getByTitle = async (group_unit = 0,name = "") => {
    return new Promise((resolve, reject) => {
        connectionDblystem.query(queries.getByTitle(), [group_unit,name], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
