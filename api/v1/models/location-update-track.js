...
const queries = require('../queries/location-update-track');
...
let getByUnitName = async (unit_name = '') => {
    return new Promise((resolve, reject) => {
        connectionDblystem.query(queries.getByUnitName(), [unit_name], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getList = async (offset, limit,unit_id, key) => {
    return new Promise((resolve, reject) => {
      connectionDblystem.query(queries.getList(offset, limit,unit_id, key), (error, result, fields) => {
        if (error) reject(error);
        else resolve(result);
      });
    });
...
let getListCount = async (unit_id, key) => {
    return new Promise((resolve, reject) => {
      connectionDblystem.query(queries.getListCount(unit_id, key), (error, result, fields) => {
        if (error) reject(error);
        else resolve(result);
      });
    });
