...
router.get(
  "/list",
  [verifyToken, routeAccessChecker("slaList")],
  async (req, res) => {
...
router.put(
  "/:id",
  [verifyToken, routeAccessChecker("slaConfiguration")],
  async (req, res) => {
    let reqData = {
    ...
    if (
      reqData.response_time_unit !== "minutes" &&
      reqData.response_time_unit !== "hours" &&
      reqData.response_time_unit !== "day" &&
      reqData.response_time_unit !== "month"
    ) {
      return res.status(400).send({
        success: false,
        status: 400,
    ...
    if (
      reqData.resolve_time_unit !== "minutes" &&
      reqData.resolve_time_unit !== "hours" &&
      reqData.resolve_time_unit !== "day" &&
      reqData.resolve_time_unit !== "month"
    ) {
      return res.status(400).send({
        success: false,
        status: 400,
